#!/bin/bash
# Retry failed experiments from Round 3
# These failed due to GPU contention (CUDA device busy)
# Using --exclusive to avoid sharing nodes
...
# Clean up failed checkpoint dirs (only have data_info.json)
for exp in sft_codealpaca_n2000_r8_lr5e-5_ep1 sft_numinamath_n2000_r8_lr5e-5_ep1_7b sft_openr1_n1000_r8_lr5e-5_ep1_7b sft_openr1_n2000_r8_lr5e-5_ep1_7b sft_openr1_n5000_r8_lr5e-5_ep1_7b; do
  if [ -d "checkpoints/${exp}" ] && [ ! -d "checkpoints/${exp}/final_adapter" ]; then
    echo "  Cleaning failed checkpoint: $exp"
    rm -rf "checkpoints/${exp}"
  fi
done

# 1. CodeAlpaca N=2000 (3B, 1 GPU)
EXP="sft_codealpaca_n2000_r8_lr5e-5_ep1"
cat > /tmp/slurm_retry_${EXP}.sh << 'SLURM_EOF'
#!/bin/bash
...
echo "=== Done ==="
SLURM_EOF
sed -i "s|SCRIPT_DIR_PLACEHOLDER|${SCRIPT_DIR}|g" /tmp/slurm_retry_${EXP}.sh
JOB_ID=$(sbatch /tmp/slurm_retry_${EXP}.sh 2>&1 | grep -oP '\d+')
echo "  Submitted ${EXP} -> Job ${JOB_ID}"

# 2-5. 7B experiments (2 GPUs each)
for exp_tuple in \
  "numinamath:2000" \
  "openr1:1000" \
  "openr1:2000" \
